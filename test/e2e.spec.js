...
test.describe('NetHack Web Runtime', () => {
  ...
  test('walks to next level', async ({ page }) => {
    ...
    // Helper: inject nav modules and run AI, returning the result
    async function runNavAttempt() {
      await page.goto(PAGE_URL, { waitUntil: 'domcontentloaded', timeout: 30000 });
      await completeCharacterCreation(page);
      await page.waitForTimeout(500);
      ...
      return await finalResult.jsonValue();
    }

    // Try up to 3 times with different map seeds
    let lastResult = null;
    for (let attempt = 1; attempt <= 3; attempt++) {
      console.log(`[TEST] === Navigation attempt ${attempt}/3 ===`);
      lastResult = await runNavAttempt();
      console.log(`[TEST] Attempt ${attempt} result:`, JSON.stringify(lastResult));

      if (lastResult.reason === 'descended') {
        break; // Success!
      }
      if (lastResult.reason === 'died') {
        break; // Death is a real failure
      }
      // 'stuck' or 'max-ticks' — retry with a new map seed
      if (attempt < 3) {
        console.log(`[TEST] Retrying with new map seed...`);
        await page.waitForTimeout(2000);
      }
    }

    expect(lastResult.reason).toBe('descended');
  });
