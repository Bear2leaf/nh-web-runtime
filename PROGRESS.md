...
**最新 200-trial 批次**: ~17% 成功 / ~35% 死亡 / ~44% 卡住 / ~2% max-ticks / ~2% 其他

> 高方差警告：100-trial 批次范围 10-25%，200-trial 批次范围 11-21%。300+ trial 才能获得稳定的统计估计。
>
> **关键发现**：commit `4af55cb` 报告的 21.5% 基线仅为单次幸运跑，重新跑 200-trial 为 **17.0%**。当前代码（HEAD）与基线统计上无显著差异（~17% vs ~17%）。
>
> **本轮实验（全部失败或无效）**：
> - `failedBoulders` 加入 BFS：14.0% 成功 / 38.5% 死亡 — 饿死风险
> - 走廊宠物后退逃逸：16.0% 成功 / 40.0% max-ticks — 创建来回振荡逃避 stuck 检测
> - 走廊振荡冷却 5→2：13.0% 成功 / 41.0% max-ticks — 同上，无进度地更频繁移动
> - 振荡打断器冷却 15→8：11.0% 成功 / 31.5% max-ticks — 同上
>
> **教训**：任何让 AI "更活跃" 的 stuck 逃逸机制，如果没有真正朝 stairs 前进，只会把 `stuck` 转化为 `max-ticks`，不提升成功率。
...
| **100次当前 (基线 8bb4387)** | **12% (12/100)** | **39% (39/100)** | **49% (49/100)** | **当前基线，验证用** |
...
| **200次基线 (4af55cb)** | **17.0% (34/200)** | **35.0% (70/200)** | **44.0% (88/200)** | **重新跑，原21.5%为方差** |
| **200次 HEAD (2320825)** | **~17% (±3%)** | **~35% (±3%)** | **~44% (±3%)** | **与基线统计相同** |
...
>
> **后续转变**: 引入 intro text 修复、pet swap burst、oscillation breaker 改进后，max-ticks 从 ~36% 降至 ~2%，但 stuck 率回升至 ~44%。瓶颈再次转为"卡住"。当前 code 与 commit 4af55cb 统计等价（~17% 成功）， variance 掩盖了小幅改进。
>
> **最新改进**:  doorway pet blocking 优化（5-tick 等待 + 后退逃逸）+ boulder 踢墙修复后，**成功率从 12% 提升至 17.5%（200-trial 合并）**，**卡住率从 49% 降至 45%**，**死亡率从 39% 降至 34%**。宠物阻塞和踢墙导致的早期 stuck 显著减少。
> 
> **本次改进**: 
...
#### 55. boulder-pet 使用 PET_CHARS 排除敌对怪物
`nav-boulder-pet.mjs` 原来用 `MONSTERS.has(nch)` 查找相邻宠物，会误把敌对怪物也当成宠物。改为 `PET_CHARS.has(nch)`，只处理真正的宠物（狗 `d`、猫 `f`、马 `u`）。避免在战斗时错误地尝试与怪物 swap，导致玩家白白挨打。

#### 56. boulder 踢墙修复 — 不再浪费回合
`nav-boulder-pet.mjs` 的 kick 逻辑原来对任何非 walkable 目标都会踢（包括墙 `|` `-`）。踢墙会导致 "You strain a muscle"，浪费回合且可能受伤。修复后：只踢向另一个 boulder `` ` `` 或怪物（它们可能移动），遇到墙直接跳过。

#### 57. doorway 宠物阻塞 5-tick 等待
原来 doorway/房间中宠物阻塞时只等待 1 tick，NetHack 宠物每 ~12 回合才移动一次，1 tick 远远不够。改为等待 5 tick burst，给宠物足够时间自行走开。通过 `_doorwayPetWaitUntil` 实现多 tick 连续等待，避免每 tick 重复检测。

#### 58. 宠物死锁后退逃逸
 doorway 中宠物阻塞且 perpendicular 方向不可走时，玩家和宠物可能形成真正死锁（例如 1-tile 宽走廊，宠物身后是墙）。新增：当 `stuckCount > 15` 且相邻有宠物时，尝试向宠物反方向（后退）移动一步，给宠物腾出空间。这是打破宠物-玩家互卡的最后手段。
...
#### 62. 走廊宠物振荡强制方向 — 不再让 explore handler 搅局
`nav-corridor.mjs` 检测到振荡后尝试与宠物 swap 4 次，然后 `return false` 让后续 handler 接管。但 `handleLevelExplore`/`handleExplore` 发送的方向往往与走廊方向相反，导致玩家和宠物在 1-tile 走廊中来回 swap，永远无法脱身。
**修复**: 
- swap 尝试阈值 4→2，更快进入强制模式
- 放弃 `return false`，改为**强制继续移动**：优先 `lastMoveDir`（swap 后指向远离宠物的方向），其次任意走廊方向 `#`，最后任意可行走方向
- 宠物搜索改用 `PET_CHARS` 而非 `MONSTERS`，避免将相邻敌对怪物误判为宠物而尝试 swap
...
### 核心教训（本轮实验得出）
> **任何让 AI "更活跃" 的 stuck 逃逸机制，如果没有真正朝 stairs 前进，只会把 `stuck` 转化为 `max-ticks`，不提升成功率。**
> 
> 具体失败模式：后退逃逸 / 冷却缩短 / 更频繁随机移动 → 玩家每几 ticks 移动一次 → `stuckCount` 永不超 1200 → 20000 ticks 后 `max-ticks`。成功率和死亡数不变。

### 高优先级（卡住 → 成功转化）
1. **宠物走廊死锁** — 当前 boulder-pet 跳过走廊 + 5-tick burst + oscillation breaker 的组合仍未解决 ~30% stuck 中的宠物问题。真正有效方案：
   - **强制后退** 不可行（已验证，导致 max-ticks 飙升）
   - **杀死宠物** (`F` + 方向) 作为最后手段：宠物 HP 低，1-3 击可清除走廊阻塞
   - **Teleport 提前**：当前 stuckCount>200 才触发，可提前到 100，但受限于 3 次上限
2. **隐藏怪物 timeout** — 当前 handler 循环 8 方向 'F' 攻击，占卡住 ~40%。已实现 40-tick teleport fallback，但 teleport 可能不可用。可行方案：
   - 80-tick 后尝试 prayer (`#pray`)
   - 120-tick 后尝试 'm' 前缀强制移动（可能挤过怪物）
3. **陷阱循环** — "Really step onto that trap?" 重复出现导致 stuck。当前 trap avoidance 仅标记已知陷阱，但 bear trap  caught 后 escape 机制（`nav-stuck.mjs` 随机方向）可能无效。
...
6. ** starvation 预防** — 少数死亡来自饥饿。确保 Weak/Fainting 时 food handler 能突破 combat/corridor 的优先级抢到食物。注意：fortune cookie 提供极少营养，可能导致无限吃循环。

### 低优先级 / 已证明无效
7. ~~房间无出口 wall search~~ — 阈值已较合理（enclosedTick > 50）
8. ~~`failedBoulders` 加入 BFS~~ — **已验证无效**：玩家避开本可推动的 boulder，导致饿死或 trapped
9. ~~走廊振荡冷却缩短~~ — **已验证有害**：把 stuck 转为 max-ticks
