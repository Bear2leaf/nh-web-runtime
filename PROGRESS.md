...
### 20次测试结果 (commit 7945d49, 2026/05/13)
| 结果 | 次数 | 百分比 |
|------|------|--------|
| **成功 (descended)** | 2/20 | 10% |
| 死亡 (game-ended) | 14/20 | 70% |
| 卡住 (stuck) | 4/20 | 20% |

### 死因分析
- jackal/fox持续攻击致死的占多数
- 战斗逃跑阈值10%太低，导致玩家HP耗尽
- 尝试提高逃跑阈值(30%)会导致0%成功率（逃跑过多）
- 需要更智能的战斗策略

### 今日主要改进
1. **楼梯冲刺** - 楼梯可见且距离≤5时优先冲向楼梯
2. **门优先探索** - 优先处理未尝试的门
3. **宠物交换节流** - 防止无限交换循环
4. **地板食物检测** - 从"You see here"消息检测
5. **on-tile monster改进** - 连续攻击时teleport
6. **wall search时enclosedTick增长** - 允许teleport触发
7. **战斗变量作用域修复** - curHp/maxHp/dx/dy作用域问题
8. **楼梯可见时不触发wall search** - 防止阻塞楼梯导航
