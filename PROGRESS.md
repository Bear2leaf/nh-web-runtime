...
## 当前瓶颈（按影响排序，基于 100-trial 14%/38%/43%）

1. **卡住 43%** — 宠物走廊阻塞（~40%）、隐藏怪物（~20%）、无出口房间（~20%）、其他（~20%）
2. **战斗死亡 38%** — 低级怪物围攻、 sickness、 starvation
3. **其他 6%** — max-ticks、WASM 崩溃、不明原因

> 注：本次改进将死亡率从 ~50% 降至 38%，但卡住率从 ~30% 升至 43%。成功率的提升来自"宁可卡住也不送死"的保守策略。下一步需在减少卡住的同时维持低死亡率。

## 下一步建议

### 高优先级（卡住 → 成功转化）
1. ~~宠物走廊死锁~~ — **已尝试** (#54 走廊等待 + #55 stuckCount>50 主动远离)。待测试验证效果。
2. **隐藏怪物 timeout** — 当前 handler 会无限 'F' 攻击不可见的怪物。`_hiddenMonsterStartTick` 字段已添加但未启用 timeout（注释认为"fight to the death"比放弃更好）。若卡住率仍高，可添加 40-60 tick timeout 后尝试向开阔方向移动。

### 中优先级（死亡 → 成功转化）
3. ~~被围检测~~ — **已实现** (#56 kiting 触发条件新增 `adjMonsterCount >= 2`)。同时保留低 HP (<30%) 撤退。
4. ** sickness 预防** — 疾病死亡占比上升。可尝试： eat 前检测 corpse 年龄（old/tainted/rotten 关键词），或直接避免吃非 starting-food 的 corpse
5. ** starvation 预防** — 少数死亡来自饥饿。确保 Weak/Fainting 时 food handler 能突破 combat/corridor 的优先级抢到食物

### 低优先级
6. **房间无出口** — wall search 阈值已较合理（enclosedTick > 50），过早触发会浪费 ticks
